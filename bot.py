...
async def handle_message(message: types.Message):
    ...
    # Добавляем обязательные для OpenRouter параметры HTTP-Referer и X-Title
    headers = {
        "Authorization": f"Bearer {OPENROUTER_KEY}",
        "Content-Type": "application/json",
        "HTTP-Referer": "https://salfett.onrender.com", 
        "X-Title": "Telegram Neuroham Bot"
    }
        ...
        async with ClientSession() as session:
            async with session.post(url, headers=headers, json=data) as response:
                ...
                if status == 200:
                    ...
                    # Если OpenRouter вернул ошибку, выводим её в логи Рендера
